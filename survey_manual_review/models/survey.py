...
class SlideChannel(models.Model):
    ...
    surveys_manual_review = fields.Many2many('survey.user_input', string="Needs Manual Review", compute="compute_surveys_manual_review")
    surveys_manual_review_count = fields.Integer(compute="compute_surveys_manual_review")
    
    def compute_surveys_manual_review(self):
        for record in self:
            surveys = self.env['survey.user_input'].search([('slide_id.channel_id','=',record.id),('manually_reviewed','=',False)])
...
class SurveyUserInput(models.Model):
    ...
    # ~ def _mark_done(self):
        ...
        # ~ if self.needs_manual_review:
           # ~ self.state = "needs_manual_review"
        # ~ return res
