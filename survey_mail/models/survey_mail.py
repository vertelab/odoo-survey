...
class SurveyTypeMail(models.Model):
    """ Template of survey.mail to attach to survey.survey. Those will be copied
    upon all surveys created in that type to ease surveys creation. """
    _name = 'survey.type.mail'
    _description = 'Mail Scheduling on Survey'
    ...
    def _prepare_survey_mail_values(self):
        ...
        return namedtuple("MailValues",
                          ['notification_type', 'interval_nbr', 'interval_unit',
...
class SurveyMailScheduler(models.Model):
    ...
    _name = 'survey.mail'
    _rec_name = 'survey_id'
    _description = 'Survey Automated Mailing'
    ...
    def set_template_ref_model(self):
        ...
        if self.notification_type == 'mail':
            record = mail_model.search([('model', '=', 'survey.user_input')], limit=1)
    ...
    mail_registration_ids = fields.One2many(
        'survey.mail.registration', 'scheduler_id',
        help='Communication related to survey registrations')
    ...
    @api.depends('survey_id.date_begin', 'survey_id.date_end', 'interval_type', 'interval_unit', 'interval_nbr')
    def _compute_scheduled_date(self):
        for scheduler in self:
            if scheduler.interval_type == 'after_sub':
                date, sign = scheduler.survey_id.create_date, 1
            elif scheduler.interval_type == 'before_survey':
                date, sign = scheduler.survey_id.date_begin, -1
            else:
    ...
    def execute(self):
        for scheduler in self:
            ...
            if scheduler.interval_type == 'after_sub':
                if self.env.context.get('survey_mail_registration_ids'):
                    new_registrations = self.env['survey.registration'].search([
                        ('id', 'in', self.env.context['survey_mail_registration_ids']),
                        ('survey_id', '=', scheduler.survey_id.id),
                    ]) - scheduler.mail_registration_ids.registration_id
                else:
                    new_registrations = scheduler.survey_id.registration_ids.filtered_domain(
                ...
                scheduler.update({
                    'mail_done': total_sent >= (scheduler.survey_id.seats_reserved + scheduler.survey_id.seats_used),
                    'mail_count_done': total_sent,
                })
            else:
                # before or after survey -> one shot email
    ...
    def _create_missing_mail_registrations(self, registrations):
        ...
        if new:
            return self.env['survey.mail.registration'].create(new)
        return self.env['survey.mail.registration']

    def _prepare_survey_mail_values(self):
        self.ensure_one()
        return namedtuple("MailValues",
                          ['notification_type', 'interval_nbr',
                           'interval_unit', 'interval_type', 'template_ref'])(
            self.notification_type,
            self.interval_nbr,
            self.interval_unit,
            self.interval_type,
    ...
    def _warn_template_error(self, scheduler, exception):
        ...
        if random.random() < 0.1666 or scheduler.interval_unit in ('now', 'hours'):
            ...
            try:
                survey, template = scheduler.survey_id, scheduler.template_ref
                emails = list(set([survey.organizer_id.email, survey.user_id.email, template.write_uid.email]))
                subject = _("WARNING: survey Scheduler Error for survey: %s", survey.name)
                body = _("""Survey Scheduler for:
  - Survey: %(survey_name)s (%(survey_id)s)
  - Scheduled: %(date)s
...
You receive this email because you are:
  - the organizer of the survey,
  - or the responsible of the survey,
...
""",
    ...
    def schedule_communications(self, autocommit=False):
        ...
        for scheduler in schedulers:
            try:
                # Prevent a mega prefetch of the registration ids of all the surveys of all the schedulers
...
class SurveyMailRegistration(models.Model):
    ...
    scheduler_id = fields.Many2one('survey.mail', 'Mail Scheduler', required=True, ondelete='cascade')
    registration_id = fields.Many2one('survey.registration', 'Attendee', required=True, ondelete='cascade')
