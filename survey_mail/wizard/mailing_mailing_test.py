    ...
    def send_mail_test(self):
        ...
        if record:
            ...
            body = mailing._render_field(
                'body_html', record.ids, compute_lang=False, options={'preserve_comments': True}
            )[record.id]
            ...
            if mailing.mailing_model_id.model == 'survey.user_input':
                full_body_soup = BeautifulSoup(body, 'html.parser')
                survey_href = full_body_soup.find('a', id='survey_button_url')
                if survey_href and survey_href.get('href') == '#':
                    survey_href['href'] = f'{record.survey_start_url}?answer_token={record.access_token}'
                full_body = Markup(full_body_soup)
        else:
