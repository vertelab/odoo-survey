...
class SurveyParticipantInvite(models.TransientModel):
    _name = 'survey.participant.invite'
    _description = 'Survey Participant Invite'
    ...
    partner_ids = fields.Many2many(
        'res.partner', 'survey_participant_invite_partner_ids', 'invite_id', 'partner_id', string='Recipients',
        domain="[ \
    ...
    def _onchange_partner_ids(self):
        if self.survey_users_login_required and self.partner_ids:
            if not self.survey_id.users_can_signup:
                ...
                if invalid_partners:
                    raise UserError(_(
                        'The following recipients have no user account: %s. You should create user accounts for them '
                        'or allow external signup in configuration.',
    ...
    def _get_answers_values(self):
        return {
            ...
            'survey_start_url': self.survey_start_url
        }
